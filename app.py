...
def compliments_results():
    ...
    wantc = request.args.get("wants_compliments")
    numc = request.args.get("num_compliments")
    compliments = random.sample(list_of_compliments, k = int(numc))
    context = {
...
def animal_facts():
    ...
    if selected_animal == "none":
        facts = "none"
    elif selected_animal != "none":
        facts = animal_to_fact.get(selected_animal)
...
def image_filter():
    ...
    if request.method == 'POST':
        ...
        input_image = save_image(image, filter_type)

        # TODO: Call `apply_filter()` on the file path & filter type
        apply_filter(input_image, filter_type)
